...
class RFQConfirmDialog(QDialog):
    ...
    def __init__(
        ...
        price_sign = "credit" if eval_price < 0 else "debit"
        price_lbl = QLabel(
        ...
        layout.addWidget(QLabel(f"<b>Threshold type:</b> {threshold_type}"))
        layout.addWidget(QLabel(f"<b>Max DD (structure):</b> ${dd_struct:,.2f}"))

        layout.addWidget(QLabel(""))
        note = QLabel(
            "<i>This dialog is informational only. Submission is not "
            "gated on drawdown limits.</i>"
        )
...
class GUIOrchestrator(QObject):
    ...
    def __init__(self, window: MainWindow) -> None:
        ...
        self._time_days: float = 7.0
        self._vol_shift: float = -0.10
        self._last_eval: Optional[StructureEvaluation] = None
    ...
    async def _evaluate(
        ...
        # Build RFQ config from current UI state
        self._rfq_config = RFQConfig(
            target_spot=target_spot if target_spot is not None else 0.0,
            threshold_type=ThresholdType.DEBIT,
            threshold_value=0.0,  # no threshold gate in GUI — informational only
            max_dd_usd_flat=-999_999.0,
            max_dd_usd_shocked=-999_999.0,
            drawdown_days=int(self._time_days) or 1,
    ...
    async def _reprice(
        ...
        # Build evaluation for RFQ submission (uses vol-shocked price at eval spot)
        marks = _build_live_marks(self._mds, parsed_legs, current_spot, config)

        self._last_eval = StructureEvaluation(
            legs=parsed_legs,
            current_spot=current_spot,
            target_spot=eval_spot,
            total_btc=tgt_shock_result.total_btc,
            total_usd=tgt_shock_result.total_usd,
            per_leg_btc=[float(v) for v in tgt_shock_result.leg_prices_btc],
            per_leg_usd=[float(v) for v in tgt_shock_result.leg_prices_usd],
            per_leg_iv=[float(v) * 100 for v in tgt_shock_result.leg_ivs],
            per_leg_forward=[float(v) for v in tgt_shock_result.leg_forwards],
            ...
            threshold_reason="GUI mode",
            dd_flat_passed=True,
            dd_shocked_passed=True,
            all_passed=True,
            deribit_marks_usd=marks,
        )
    ...
    async def _submit_rfq(self) -> None:
        ...
        dlg = RFQConfirmDialog(
            ...
            eval_price=ev.total_usd,
            dd_struct=ev.max_dd_flat,
            spot_value=spot_value,
            spot_label=spot_label,
            threshold_type=self._rfq_config.threshold_type.value
            if self._rfq_config
            else "N/A",
            parent=self._win,
        )
